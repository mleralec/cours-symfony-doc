...
Actuellement, les seules méthodes HTTP supportées par les formulaires HTML sont GET et POST. Pour contrer ce problème, Symfony utilise un input de type `hidden`, avec un *name* qui vaut `_method` et la valeur égale à la méthode HTTP voulu.
...
Rediriger ensuite sur la route `article.index`.
